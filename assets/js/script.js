...
function loadAddThis() {
  if ($('.addthis_inline_share_toolbox').length > 0) {
    var consentObject = getCookie("cookie_consent_level");
    ...
    if (consentObject == null) {
      ...
      var cookieConsent = JSON.parse(decodeURIComponent(getCookie("cookie_consent_level")));
        $('.addthis_inline_share_toolbox').css('display','inherit')
        if ($('.sharer').length > 0) {
          $('.sharer').css('display','inherit')
        }
        $('.share').css('display','inherit')

      if(cookieConsent.targeting) {
        if(typeof addthis !== 'undefined') { addthis.init(); addthis.layers.refresh(); }
      } else {
        $('.addthis_inline_share_toolbox').css('display','none')
        if ($('.sharer').length > 0) {
          $('.sharer').css('display','none')
      ...
      if(!cookieConsent.targeting && !cookieConsent.functionality) {
        $('.share').css('display','none')
      }
    }
  }
...
function loadDisqus() {
    ...
    if (consentObject == null) {
        ...
        $('.dqc-count').css('display','none')
    }
    else {
      var cookieConsent = JSON.parse(decodeURIComponent(getCookie("cookie_consent_level")));

      if(cookieConsent.functionality) {
        $('.disqus').css('display','inherit')
        $('.dqc-count').css('display','inherit')

        disqusLoader('.disqus', 
        {
          scriptUrl: '//tensevntysevn.disqus.com/embed.js',
          laziness: 1,
          throttle: 250,
          disqusConfig: function()
          {
            this.page.title       = document.getElementsByTagName("title")[0].innerHTML;
            this.page.url         = window.location.href;
            this.page.identifier  = window.location.href;
          }
        });
        window.DISQUSWIDGETS = undefined;
        $.getScript("https://tensevntysevn.disqus.com/count.js");
      } else {
        $('.disqus').css('display','none')
        $('.dqc-count').css('display','none')
      }
...
$(document).ready(function () {
  ...
  // Add navbar background if URL has hash and it is scrolled over
  ...
  $(".article-image-container").magnificPopup({
    type: "image",
    removalDelay: 300,
    preloader: true,
    fixedContentPos: false,
